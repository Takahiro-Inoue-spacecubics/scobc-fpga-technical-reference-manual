...
|              0x4F042080 | SYSMON_BHM_I2CPSCR      | Board Health I2C Prescale Setting Register                | 0x00000077 |
...
初期設定値は、システムクロックが48MHzの場合のFast-mode(400Kb/s)でのI2C通信をターゲットとして設定されています。
...
システムクロックが96MHz, 48MHz, 24MHzにおいて、Fast-mode(400Kb/s)で通信する場合の、本レジスタの設定値とI2C通信の各タイミング値の例を以下に示します。

#+CAPTION: Fast-mode(400Kb/s)通信時のBoard Health I2C Prescale Setting Registerの設定例とI2Cタイミング (システムクロックのサイクル数)
| Parameter                           | System Clock=96MHz     | System Clock=48MHz[default] | System Clock=24MHz    |
|-------------------------------------+------------------------+-----------------------------+-----------------------|
| SYSMON_BHM_CLKPSC[15:0] 設定値      | 0x00EF [Divide by 240] | 0x0077 [Divide by 120]      | 0x003B [Divide by 60] |
| START Hold Timing(tHDSTA)           | 120 Cycle [1.25us]     | 60 Cycle [1.25us]           | 30 Cycle [1.25us]     |
| STOP Setup Timing(tSUSTO)           | 120 Cycle [1.25us]     | 60 Cycle [1.25us]           | 30 Cycle [1.25us]     |
| Repeated START Setup Timing(tSUSTA) | 120 Cycle [1.25us]     | 60 Cycle [1.25us]           | 30 Cycle [1.25us]     |
| Clock High Timing(tHIGH)            | 115 Cycle [1.20us]     | 55 Cycle [1.15us]           | 25 Cycle [1.04us]     |
| Data Hold Timing(tHDDAT)            | 5 Cycle [0.05us]       | 5 Cycle [0.10us]            | 5 Cycle [0.21us]      |
| Data Setup Timing(tSUDAT)           | 120 Cycle [1.25us]     | 60 Cycle [1.25us]           | 30 Cycle [1.25us]     |
| Clock Low Timing(tLOW)              | 125 Cycle [1.30us]     | 65 Cycle [1.35us]           | 35 Cycle [1.46us]     |
| Bus Free Timing(tBUF)               | 125 Cycle [1.30us]     | 65 Cycle [1.35us]           | 35 Cycle [1.46us]     |
...
I2C通信のビットレートを決めるタイミングパラメータの初期設定は、システムクロックが48MHzの場合のFast-mode(400Kb/s)をターゲットに設定されています。
システムクロックの動作周波数が48MHz、かつ、I2C通信のビットレートがFast-mode(400Kb/s)から変更が無い場合は、以下の手順[1]を省略することができます。

また、I2C通信エラー発生による再実行アクセスを含めたI2Cアクセス実行回数の初期設定は3回に設定されています。
...
7: Board Health Monitoring Access Control Registerの全てのxxxxMONIENビットを"1"に設定し、デバイスの測定データの取得を開始します。

Monitoring Enableのセット後は、定期的に測定データがデバイスから読み出され各Monitor Registerへ格納されます。
Monitor Registerの更新周期は、各デバイスのデータ測定の待機時間に合わせて設定される、General Purpose Timerからのイベントトリガの発行周期に同期します。

8: Current Voltage Monitorの測定データの更新周期に合わせて、各デバイスおよび各Channelのシャント電圧、バス電圧の測定データを読み出します。

9: Temperature Sensorの測定データの更新周期に合わせて、各デバイスの温度の測定データを読み出します。
