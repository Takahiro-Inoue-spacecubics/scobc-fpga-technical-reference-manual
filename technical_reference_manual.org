...
割り込みを発生させるタイマーの時間を設定する出力比較チャネルを、Global Timerは 4つ、Software Interrupt Timerと Hardware Interrupt Timerはそれぞれ 8つずつ持っています。
Global Timerと Software Interrupt Timerは、全ての出力比較チャネルを CPUへの割り込み要因として任意に使用することができます。
Hardware Interrupt Timerは、Channel 1を除き、それぞれの出力比較チャネルが特定の IPコアのイベントトリガに使用されます。

Hardware Interrupt Timerの出力チャネルと、接続されている IP Core(機能)との対応を以下に示します。

#+CAPTION: Hardware Interrupt Timerの出力比較チャネル対応
| Output Compare Channel | to IP Core                                                              |
|------------------------+-------------------------------------------------------------------------|
|                      1 | Reserved (Restartモード時のタイマーの周期設定として使用する)            |
|                      2 | System Monitor (Current Voltage Monitorへの データ読み出しアクセス要求) |
|                      3 | System Monitor (Temperature Sensorへの データ読み出しアクセス要求)      |
|                    4-8 | Reserved (Unused)                                                       |
...
|   bit | Symbol      | Field                              | Description                                                                                                                                                                                                                                                                                                                       | R/W |
|-------+-------------+------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----|
| 31:16 | MEMSCRCYC   | Memory Scrubing Wait Cycle         | メモリスクラビングによる読み出しアクセスの Wait Cycleを設定するビットです。0: 設定禁止 1: 1 Cycle 2: 2 Cycle ... 65535: 65535 Cycle 初期状態の Wait Cycleは、1791 Cycleに設定されています。                                                                                                                               | R/W |
...
SC OBCモジュールには、FPGA Configurationデータを格納するためのNOR Flashメモリと、運用データを格納するための NOR Flashメモリ 及び FeRAMが、それぞれ2個ずつ搭載されています。
これらのメモリとの通信を行う場合には、QSPI Controllerを使用します。

SC OBCモジュールに実装される メモリの型番は以下の通りです。
各メモリの仕様については、ベンダーからリリースされている Datasheetを参照してください。

| Memory                                     | Vendor               | Part Number | SC OBC搭載数 |
|--------------------------------------------+----------------------+-------------+--------------|
| Configurationデータ格納用 NOR Flash Memory | infineon (旧Cypress) | S25FL256L   | 2 個         |
| データ格納用 NOR Flash Memory              | infineon (旧Cypress) | S25FL256L   | 2 個         |
| データ格納用 FeRAM                         | infineon (旧Cypress) | CY15B104QSN | 2 個         |

** レジスタ詳細
QSPI Controllerは、各メモリにアクセスするため 3個搭載されています。
同じ種類のメモリへのアクセスは一つの QSPI Controllerで行い、Chip Selectの制御により排他でアクセスを行うことが出来ます。
...
Data Memory Access QSPI Controllerと FeRAM Access QSPI Controllerは、Chip Select信号の制御を SPISSCTL1ビット または SPISSCTL2ビットの設定により行います。
これらのビットのうち、SPIアクセスを行ういずれか 1つのメモリ Portのビットのみを "1"にセットして、SPIアクセスの操作を行ってください。
SPISSCTL1ビットと SPISSCTL2ビットを、同時に "1"にセットすることは禁止動作となります。

Configurationデータ格納用 NOR Flash Memoryへ SPIアクセスを行う場合は、アクセスを行う Portに関わらず、常に SPISSCTL1ビットにより Chip Select信号を制御します。
このため、Configuration Memory Access QSPI Controllerには SPISSCTL2ビットは実装されていません。
Configurationデータ格納用 NOR Flash Memoryの SPIアクセスを行う Portの選択は、System Registerの Configuration Memory Registerの設定により行うことができます。
詳細は、System Registerの「Configuration Memory Register」の章を参照してください。
...
|     1 | SPISSCTL2 | SPI Port 2 SS Control | Port 2側のデバイスへの SS(Chip Select)信号を制御するためのビットです。 0: デバイス Port 2のSS信号をインアクティブ状態に設定します 1: デバイス Port 2のSS信号をアクティブ状態に設定します | R/W |
|     0 | SPISSCTL1 | SPI Port 1 SS Control | Port 1側のデバイスへの SS(Chip Select)信号を制御するためのビットです。 0: デバイス Port 1のSS信号をインアクティブ状態に設定します 1: デバイス Port 1のSS信号をアクティブ状態に設定します | R/W |
...
以下手順は、Port 1側のメモリへアクセスするときのレジスタ設定例を示しています。
Configurationデータ格納用 NOR Flash Memoryを除き、Port 2側のメモリへアクセスする場合の設定は、QSPI Access Control Registerの下位16ビットの書き込み値を、0x0001 から 0x0002に置き換えて下さい。
...
QSPI TX Data Registerに書き込まれたデータからSPIデバイスに順次送信されます。
続けて、QSPI RX Data Registerに 4 Byte分の書き込みを行うことで、8 Cycleのダミーサイクル区間で IO信号を入力モードにして SPIクロックを駆動します。

E: QSPI RX Data Registerの読み出しを 4 Byte分行い、ダミーサイクル区間に RX FIFOに格納されたデータの読み出しを行います。
ダミーサイクル区間に格納されたデータは全て無効なデータとなります。
...
I2C Master Controllerは、これら 2つのインタフェースの内 External I2Cの制御を行います。
