...
System Registerにある Code Memory Select Register の ITCMENビットを変更してシステムリセットが発行されている最中も、このレジスタには常に最新のステートの状態が更新されます。

SEM Controllerはシステムリセットの影響を受けず、FPGAの Configratoin完了後は常に Configurationデータの状態を監視しています。
一方で、System Monitor Interrupt Status Registerはシステムリセットにより初期化が行われます。
このため、システムリセットが発行されている最中に SEM Controllerの割り込みイベントが発生した場合、ユーザーはこの時発生した割り込みイベントを知ることができません。
したがって、システムリセットの発行中における Configurationデータの異常検出の有無は、システムリセットの解除後にこのレジスタを読み出すことにより確認してください。
...
このレジスタはシステムリセットによる初期化が行われません。
System Registerにある Code Memory Select Register の ITCMENビットを変更してシステムリセットが発行されている最中も、このレジスタには常に最新のエラー訂正数が更新されます。
...
このレジスタは、SC OBC FPGAの Configuration後に一度だけ初期化されます。
System Registerにある Code Memory Select Registerの ITCMENビットが変更された時に発行されるシステムリセットでは、このレジスタはクリアされません。
