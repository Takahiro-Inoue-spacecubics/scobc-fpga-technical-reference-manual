...
SC OBC FPGAのBoot Statusを示すレジスタです。
SC OBC FPGAのConfigurationが完了した時に FPGA Configuration Registerの Boot History Status Register(Address: 0x16)にアクセスを行い、読み出した値をこのレジスタに格納します。

このレジスタの Bit1 は、現在起動しているConfiguration DataのFALLBACKステータスが格納されます。
Bit1 が '0' の場合は通常のConfiguration Data、Bit1 が '1' の場合はFALLBACKのConfiguration Dataにより起動していることを示します。
FPGAのマルチブート機能を使用している場合、ユーザーはこのレジスタを読み出すことによって、現在起動しているConfiguration Dataを知ることができます。

FALLBACKステータスビット以外に関する Boot History Status Registerの詳細は AMD (旧 Xilinx社) のドキュメント (UG470: 7シリーズ コンフィギュレーション ユーザーガイド)を参照してください。

このレジスタはシステムリセットによる初期化が行われません。
Code Memory Select Register の ITCMENビットを変更してシステムリセットが発行された場合も、SC OBC FPGAの起動が完了した時に格納したBoot Statusの値が保持されます。

#+CAPTION: Boot Status Register ビットフィールド
|  bit | Symbol  | Field       | Description                                                            | R/W |
|------+---------+-------------+------------------------------------------------------------------------+-----|
| 31:0 | BOOTSTS | Boot Status | Boot History Status Registerから読み出した値が格納されるレジスタです。 | RO  |
