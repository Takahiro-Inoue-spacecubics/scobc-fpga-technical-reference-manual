...
|     0 | CAN_TRNSDN  | CAN Message Transmit Done        | メッセージを送信したことを示すビットです。データフレームまたはリモートフレームの送信が正常に完了し、送信したメッセージ識別子 (IDR)が TXCOMPID FIFOに格納された時、本ビットが "1"にセットされます。本ビットは TXCOMPID FIFOが Empty状態になるとクリアされます。                                                       | RO   |
